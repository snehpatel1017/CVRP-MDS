    ...
    int *g_workspace, int *Iteration_counter, double clock_rate_khz)
{
    ...
    if (tid == 0)
    {
        printf("Thread %d: MST_copy %f seconds.\n", tid, MST_copy_time);
        printf("Thread %d: shuffle %f seconds.\n", tid, shuffle_time);
        printf("Thread %d: shortcut %f seconds.\n", tid, shortcut_time);
        printf("Thread %d: convertTo_route %f seconds.\n", tid, convertTo_route_time);
        printf("Thread %d: calcost %f seconds.\n", tid, calcost_time);
        printf("Thread %d: globalupdate %f seconds.\n", tid, globalupdate_time);
    }
}
