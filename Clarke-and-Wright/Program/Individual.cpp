...
void Individual::evaluateCompleteCost(const Params &params)
{
	...
	for (int r = 0; r < params.nbVehicles; r++)
	{
		if (!chromR[r].empty())
		{
			double distance = params.getDist(0, chromR[r][0]);
			...
			{
				distance += params.getDist(chromR[r][i - 1], chromR[r][i]);
			...
			distance += params.getDist(chromR[r][chromR[r].size() - 1], 0);
...
Individual::Individual(Params &params, std::vector<std::vector<int>> &sol) : Individual(params)
{
	for (int r = 0; r < sol.size(); r++)
	{
		...
		{
			chromT.push_back(x);
			chromR[r].push_back(x);
		}
	}
	evaluateCompleteCost(params);
	std::cout << eval.penalizedCost << "\n";
}
