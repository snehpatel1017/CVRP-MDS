...
__device__ int get_lower_bound(const double *angles, int n, float target)
{
    int left = 1, right = n;
    while (left <= right)
    {
    ...
    double theta)
{
    ...
    for (int i = tid; i <= limit; i += total_threads)
    {
        ...
        // --- SWITCH BACK TO FLOAT FOR GPU PERFORMANCE ---
        // 1. Determine the angle of the tail

        float angle_back = (float)angles[back_i];
        if (angle_back < 0.0f)
            angle_back += TWO_PI;

        // 2. Determine the angle of the head
        float angle_front = (float)sorted_angles[i];
        if (angle_front < 0.0f)
            angle_front += TWO_PI;

        // 3. Find the mathematical min and max of the two angles
        float a = min(angle_front, angle_back);
        float b = max(angle_front, angle_back);

        float min_angle, max_angle, span;

        // 4. Circular logic: Determine the true left and right ends

        if (b - a <= PI_VAL)
        {
            min_angle = a - (float)theta;
            max_angle = b + (float)theta;
        }
        else
        {
            min_angle = b - (float)theta;
            max_angle = a + (float)theta + TWO_PI;
        }

        span = max_angle - min_angle;

        // 5. FIXED SPLIT LOGIC: Prevent overlapping ranges!
        float range1_min, range1_max, range2_min, range2_max;

        if (span >= TWO_PI)
        {
            // The search window covers the whole circle! Search everywhere exactly once.
            range1_min = 0.0f;
            range1_max = TWO_PI;
            range2_min = -1.0f;
            range2_max = -1.0f;
        ...
        {
            // Initialize defaults
            range1_min = min_angle;
            range1_max = max_angle;
            range2_min = -1.0f;
            range2_max = -1.0f;
            ...
            if (min_angle < 0.0f)
            {
                range1_min = 0.0f;
                range1_max = max_angle;
                range2_min = min_angle + TWO_PI;
                range2_max = TWO_PI;
            }
            else if (max_angle >= TWO_PI)
            {
                range1_min = min_angle;
                range1_max = TWO_PI;
                range2_min = 0.0f;
                range2_max = max_angle - TWO_PI;
            }
        }

        // Evaluate candidate using the parallel index
        auto evaluate_candidate = [&](int j)
        {
            if (j == i)
                return;
            ...
            // --- CRITICAL PERFORMANCE FIX: CALCULATE ONCE ---
        ...
        // --- SEARCH RANGE 1 ---
        // Notice we pass 'last_index' instead of 'num_total_customers'
        int start_idx1 = get_lower_bound(sorted_angles, last_index, range1_min);
        for (int idx = start_idx1; idx <= last_index && sorted_angles[idx] <= range1_max; ++idx)
        {
            evaluate_candidate(idx);
        }

        // --- SEARCH RANGE 2 (If wrap-around occurred) ---
        if (range2_min >= 0.0f)
        {
            int start_idx2 = get_lower_bound(sorted_angles, last_index, range2_min);
            for (int idx = start_idx2; idx <= last_index && sorted_angles[idx] <= range2_max; ++idx)
            {
                evaluate_candidate(idx);
            }
        }
