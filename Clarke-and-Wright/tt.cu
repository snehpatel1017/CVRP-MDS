...
__global__ void cost_calculation(const node_t *customer_route_map,
                                 const node_t *next_customer,
                                 const Point *nodes,
                                 const weight_t *dist_to_depot,
                                 unsigned int last_index,
                                 weight_t *Total_Cost)
{
    ...
    for (int i = tid; i <= last_index; i += total_threads)
    {
        if (customer_route_map[i] == -1)
            break;
        node_t head = customer_route_map[i];
        weight_t route_cost = 0.0;
        if (head != DEPOT)
        {
            route_cost += dist_to_depot[head];
            node_t current_node = head;
            while (next_customer[current_node] != DEPOT)
            {
                node_t next_node = next_customer[current_node];
                route_cost += device_euclidean_dist(nodes[current_node], nodes[next_node]);
                current_node = next_node;
            }
            route_cost += dist_to_depot[current_node];
        }
        printf("Route %d has cost %f\n", head, route_cost);
        atomicAdd(Total_Cost, route_cost);
    }
}
...
std::vector<std::vector<node_t>> parallel_savings_algorithm(const VRP &vrp)
{
    ...
    for (int i = 1; i <= NUM_CUSTOMERS; ++i)
    {
        ...
        h_route_head[i] = i;
        h_route_tail[i] = i;
        h_total_cost += 2 * vrp.dist_to_depot[i]; // Initial cost is going to the customer and returning to depot
    }
    costs.push_back(h_total_cost);
    h_total_cost = 0.0;
    ...
    while (true)
    {
        ...
        // cost_calculation<<<numBlocks, threadsPerBlock>>>(
        //     d_customer_route_map,
        //     d_next_customer,
        //     d_nodes,
        //     d_dist_to_depot,
        ...
        // checkCudaErrors(cudaDeviceSynchronize());
        // h_total_cost = 0;
        // checkCudaErrors(cudaMemcpy(&h_total_cost, d_total_cost, sizeof(weight_t), cudaMemcpyDeviceToHost));
        // costs.push_back(h_total_cost);
    }
    std::cout << "loop ended\n";
