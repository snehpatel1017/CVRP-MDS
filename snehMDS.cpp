...
public:
    ...
    weight_t get_dist(node_t i, node_t j, bool isRound = true) const
    {
        ...
        return ((params.toRound && isRound) ? round(w) : w);
    }
...
std::vector<std::vector<Edge>>
PrimsAlgo(const VRP &vrp)
{
    auto N = vrp.size;

    std::vector<weight_t> key(N, INT_MAX);
    std::vector<node_t> parent(N, -1);
    ...
    key[0] = 0.0;
    node_t u_node = -1;
    weight_t dist_u = INT_MAX;
    weight_t dist_v = INT_MAX;
    node_t v_node = -1;
    ...
    for (size_t i = 0; i < N; i++)
    {
        ...
        for (v_node = 0; v_node < N; v_node++)
        {
            if (visited[v_node])
                continue;
            dist_v = vrp.get_dist(u_node, v_node, false) + dist_u;
            if (dist_v < key[v_node])
            {
                key[v_node] = dist_v;
    ...
    for (u_node = 0; u_node < N; u_node++)
    {
        ...
        if (v_node == -1)
            continue;
        weight_t w = vrp.get_dist(u_node, v_node, false);
        nG[u_node].push_back(Edge(v_node, w));
        nG[v_node].push_back(Edge(u_node, w));
    }
    return nG;
}
