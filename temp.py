...
url = 'https://drive.google.com/uc?id=1wKbPNHYLsUWRu2nEmT4wIRHfNPKUj4Dv'

gdown.download(url, "inputs.zip", quiet=False)
...
--------------------------------------------------
Total Solution Cost: 4751858.71
...
Problem File: /content/inputs/Flanders2.vrp
--------------------------------------------------
Total Solution Cost: 4749191.94
Number of Routes:   257
Total Time Taken:    2043.94 seconds
Solution Validity:   VALID
--------------------------------------------------
...
struct Saving
{
    volatile unsigned long long int id;
...
__global__ void find_best_saving_kernel(
    ...
    unsigned long long int *best_saving_id_storage,
    ...
    demand_t capacity)
{
    ...
    long long local_best_id = -1;

    for (long long curr = tid; curr < total_pairs; curr += total_threads)
    {
        int i = get_pair_mathematical(curr, n);
        ...
        if (saving_value_1 > local_best_saving_value)
        {
            local_best_saving_value = saving_value_1;
            local_best_id = ((unsigned long long)i << 32) | (unsigned int)j;
        }
        if (saving_value_2 > local_best_saving_value)
        {
            local_best_saving_value = saving_value_2;
            local_best_id = ((unsigned long long)j << 32) | (unsigned int)i;
        }
    }
    best_saving_id_storage[tid] = local_best_id;
...
{
    long long tid = blockIdx.x * blockDim.x + threadIdx.x;
    unsigned long long int local_best_i = best_saving_id_storage[tid];
    if (best_saving_value_storage[tid] == best_saving_out->value)
    {
        atomicMin((unsigned long long int *)&(best_saving_out->id), local_best_i);
    }
}

__global__ void update_gpu_mempory(
    unsigned int last_pointer,
...
std::vector<std::vector<node_t>> parallel_savings_algorithm(const VRP &vrp)
{
    ...
    node_t *d_route_head;
    node_t *d_route_tail;
    unsigned long long int *d_best_saving_id_storage;
    ...
    checkCudaErrors(cudaMalloc(&d_best_saving_id_storage, (40 * 1024) * sizeof(unsigned long long int)));
    ...
    Saving h_best_saving_init = {ULLONG_MAX, 0};
    ...
    int id = 0;
    int pre_i = -1, pre_j = -1;
    unsigned int last_pointer = NUM_CUSTOMERS;
    while (true)
    {
        // std::cout << id++ << "\n";
        ...
        find_best_saving_kernel<<<numBlocks, threadsPerBlock>>>(
            ...
            d_best_saving_id_storage, d_best_saving_value_storage,
            d_best_saving_out, last_pointer, CAPACITY);

        update_best_node_i<<<numBlocks, threadsPerBlock>>>(
            d_best_saving_id_storage, d_best_saving_value_storage, d_best_saving_out);
        ...
        unsigned int i = (unsigned int)((h_result.id) >> 32);
        unsigned int j = (unsigned int)((h_result.id) & 0xFFFFFFFFULL);
    ...
    checkCudaErrors(cudaFree(d_best_saving_id_storage));
    checkCudaErrors(cudaFree(d_best_saving_value_storage));
