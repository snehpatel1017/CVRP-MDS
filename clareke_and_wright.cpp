...
// START: CLARKE AND WRIGHT SAVINGS HEURISTIC
struct Saving
{
    node_t i;
    node_t j;
...
ClarkeAndWright(const VRP &vrp)
{
    const size_t n_customers = vrp.getSize() - 1;
    if (n_customers == 0)
        return {};

    // 1. Calculate savings for all pairs of customers (i, j)
    std::vector<Saving> savings;
    savings.reserve(n_customers * (n_customers - 1) / 2);

    for (node_t i = 1; i <= n_customers; ++i)
    {
        for (node_t j = i + 1; j <= n_customers; ++j)
        {
            weight_t saving_value = vrp.get_dist(DEPOT, i) + vrp.get_dist(DEPOT, j) - vrp.get_dist(i, j);
            savings.push_back({i, j, saving_value});
        }
    }

    // 2. Sort savings in descending order
    std::sort(savings.begin(), savings.end(), std::greater<Saving>());

    // 3. Initialize routes: one route for each customer
    std::vector<std::vector<node_t>> routes(n_customers);
    std::vector<demand_t> route_demands(n_customers);
    std::vector<node_t> customer_to_route_map(vrp.getSize());

    for (size_t i = 0; i < n_customers; ++i)
    {
        node_t customer_id = i + 1;
        routes[i] = {customer_id};
        route_demands[i] = vrp.node[customer_id].demand;
        customer_to_route_map[customer_id] = i;
    }
    ...
    for (const auto &s : savings)
    {
        ...
        size_t route_idx_i = customer_to_route_map[i];
        size_t route_idx_j = customer_to_route_map[j];

        // Only merge if they are in different routes
        if (route_idx_i != route_idx_j)
        {
            // Check if capacity constraint is respected
            if (route_demands[route_idx_i] + route_demands[route_idx_j] <= vrp.getCapacity())
            {

                // i must be at the end of its route, and j at the start of its route, or vice versa
                bool i_is_endpoint = (routes[route_idx_i].back() == i);
                bool j_is_startpoint = (routes[route_idx_j].front() == j);

                bool j_is_endpoint = (routes[route_idx_i].back() == j);
                bool i_is_startpoint = (routes[route_idx_j].front() == i);

                if (i_is_endpoint && j_is_startpoint)
                {
                    // Merge j's route into i's route
                    routes[route_idx_i].insert(routes[route_idx_i].end(), routes[route_idx_j].begin(), routes[route_idx_j].end());
                    route_demands[route_idx_i] += route_demands[route_idx_j];

                    // Update map for all customers in merged route
                    for (node_t cust : routes[route_idx_j])
                    {
                        customer_to_route_map[cust] = route_idx_i;
                    }
                    routes[route_idx_j].clear();
                    route_demands[route_idx_j] = 0;
                }
                else if (j_is_endpoint && i_is_startpoint)
                {
                    // Merge i's route into j's route
                    routes[route_idx_j].insert(routes[route_idx_j].end(), routes[route_idx_i].begin(), routes[route_idx_i].end());
                    route_demands[route_idx_j] += route_demands[route_idx_i];

                    for (node_t cust : routes[route_idx_i])
                    {
                        customer_to_route_map[cust] = route_idx_j;
                    }
                    routes[route_idx_i].clear();
                    route_demands[route_idx_i] = 0;
                }
            }
        }
    }

    // 5. Collect final routes (remove empty ones)
...
// END: CLARKE AND WRIGHT SAVINGS HEURISTIC
...
int main(int argc, char *argv[])
{
    ...
    if (argc < 2)
    {
        std::cout << "parMDS version 1.2 (Clarke & Wright)" << '\n';
    ...
    // =========================================================================
    // MODIFICATION: Replace MST + DFS with Clarke & Wright Savings Heuristic
    // =========================================================================

    // Call Clarke & Wright to get the initial solution
    auto initialRoutes = ClarkeAndWright(vrp);
    ...
    // The large randomization loop is removed because the Savings Heuristic is deterministic.
    ...
    uint64_t elapsed_initial_ns = std::chrono::duration_cast<std::chrono::nanoseconds>(end_initial - start).count();
    double timeForInitialSolution = (double)(elapsed_initial_ns * 1.E-9);
    ...
    uint64_t elapsed_post_ns = std::chrono::duration_cast<std::chrono::nanoseconds>(end_total - start_postprocess).count();
    double timeForPostProcessing = (double)(elapsed_post_ns * 1.E-9);

    uint64_t elapsed_total_ns = std::chrono::duration_cast<std::chrono::nanoseconds>(end_total - start).count();
    double total_time = (double)(elapsed_total_ns * 1.E-9);
    ...
    // Print final results
    std::cout << argv[1] << " ";
    std::cout << "Initial Cost (C&W) = " << initialCost << ',';
